...
4. **A test attack is detected** — repeated wrong-password attempts against a domain account (a mini
   "someone's guessing the password" scenario; run from an admin PowerShell session on DC01 using
   `ValidateCredentials()`, not an interactive WS01 lock-screen attempt like
   [incident-writeups/01-bruteforce.md](incident-writeups/01-bruteforce.md) — same detection chain,
...
7. **The alert actually reaches TheHive, not just the Wazuh dashboard** — this is the step the first
   ...
   10 failed attempts: `60204` fired, `custom-w2thive` picked it up, and a real TheHive **alert**
   was created (`docker exec <manager> tail /var/ossec/logs/integrations.log` shows `created TheHive
   alert: {...}`, cross-checked against TheHive's own API). Note the precise claim: this is a
   TheHive *alert*, not yet a *case* — promoting an alert to a case is a manual analyst step in
   TheHive, and the `custom-w2thive` service account only holds `manageAlert/create` permission
   anyway, by design. See
...
- The alert-to-TheHive pipeline is proven, not assumed: a real TheHive alert exists for a real
